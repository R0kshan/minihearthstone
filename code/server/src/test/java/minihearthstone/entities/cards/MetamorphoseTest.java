...
public class MetamorphoseTest{
    ...
    /* TODO : Fix this
     * Test for the method {@link Metamorphose#specialSkillOnServant(String, Long, GameBoard)}
     
    @Test
    public void testSpecialSkillOnServant(){
        String c1Id = c1.getId().toString();
        when(gameBoard.findCardOnGroundByUniqueId(CL1ID, c1.getId().toString())).thenReturn(c1);
        metamorphose.specialSkillOnServant(c1Id, CL1ID, gameBoard);
        verify(gameBoard).getOpponentPlayer(CL1ID);
    }
*/
