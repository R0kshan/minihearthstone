...
@Entity
public class Consecration extends AbstractSpecificCard {

    public Consecration() {
        setName("Consecration");
    }
    ...
    public void specialSkill(Long clientId, GameBoard gameBoard) {
        ...
        for (Card servant : opponentServants) {
            // servant.setLifepoints(servant.getLifepoints()-2);
            // gameBoard.addCardUniqueIdToAttackedServant(servant.getUniqueId());

            /*int remainingDamage = 2 - servant.getDefense();
            if (remainingDamage >= 0) {
                ...
                servant.setDefense(servant.getDefense() - 2);
                gameBoard.addCardUniqueIdToAttackedServant(servant.getUniqueId());
            }*/

            this.inflictDamageToServant(servant, 2, gameBoard);
        }
	}
